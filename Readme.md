...
- `consecutive_stops`: Consecutive stop-loss breaches recorded in current session (triggers halt if $\ge 2$).
...
| 4     | `vix_spike_detected`                          | `signals.get("vix_spike_detected") == True`                                             | India VIX jumped$\ge 15\%$ intraday; short premium tail-risk explodes.                                                        |
...
| 9     | `entry_cooldown_remaining`                    | Time since last action$< \text{cooldown}$ (`ENTRY_COOLDOWN_MIN = 10`)                 | Prevents immediate revenge entries after an exit.                                                                               |
| 10    | `no_material_change_since_exit`               | $\|\text{spot} - \text{last\_exit\_spot}\| < \text{threshold}$ (40 to 80 pts)           | Prevents re-entering the exact same price zone without auction displacement.                                                    |
...
| 13    | `stop_cooldown_remaining`                     | Time since last stop$< \text{STOP\_COOLDOWN\_MAP}$ (30 to 45 min)                       | Enforces emotional/tape reset period after a stop-out (`CLOSE_STOP: 30m, CLOSE_ADX: 45m, CLOSE_VWAP: 20m, CLOSE_DELTA: 30m`). |
| 14    | `spot_velocity_too_fast`                      | 5-minute spot displacement$> \text{threshold}$ (e.g. 50 pts)                            | Prevents selling credit in front of an aggressive price locomotive.                                                             |
...
| 21    | `dte_above_max` / `dte_requires_confidence` | $\text{DTE} > \text{max\_dte}$ (4) or $\text{DTE} \ge 4$ with confidence $\ne$ HIGH | Protects against zero-theta far-dated contracts.                                                                                |
| 22    | `day_move_used_no_edge`                       | Range move consumed$\ge 75\%$ of opening straddle                                       | Expected remaining intraday price movement is exhausted.                                                                        |
| 23    | `insufficient_minutes_before_exit`            | Mins to hard exit$< \text{required}$ (50 to 90 min)                                     | Must allow adequate time for theta decay to outpace friction.                                                                   |
...
- **Two-Way Auction Positioning Override (`PATCH_V31`)**: If `signals["two_way_auction"]` is true and session range $\ge 85$ points:
  - If range location $\ge 0.80$: Overrides `final_regime` to `PREMIUM_SELL_BEAR` (High Fade).
  - If range location $\le 0.20$: Overrides `final_regime` to `PREMIUM_SELL_BULL` (Low Fade).
...
- The engine calculates session range position via `_fade_range_pos(signals)`:
  $$
  \text{loc} = \frac{\text{spot} - \text{eff\_lo}}{\text{eff\_hi} - \text{eff\_lo}}
  $$

  (Where `eff_hi` and `eff_lo` remove open-spike wicks when the spike gap $\ge 15\text{ pts}$).
- If $\text{session\_range} \ge 85\text{ pts}$ and $\text{DTE} \ge 1$:
  - If $\text{loc} \ge 0.85$: Selects **`BEAR_CALL_SPREAD`** (`afternoon_high_fade = True`).
  - If $\text{loc} \le 0.15$: Selects **`BULL_PUT_SPREAD`** (`afternoon_low_fade = True`).
  - If $0.15 < \text{loc} < 0.85$: Returns **`NO_TRADE`** (`two_way_auction_wait_for_extreme`).

#### 3. Iron Butterfly Resolution (0DTE Pin Spec):

An Iron Butterfly (selling ATM Put + ATM Call, buying wings) is strictly restricted:

- Requires $\text{DTE} \in (0, 1)$ (typically 0DTE).
- `or_condition` must be `VERY_NARROW` or `NARROW`.
- $\text{ADX}_{15} < 20$ (flat volatility, absent momentum).
- `current_time` $< 12:00\text{ IST}$ (pre-midday entry).
- $\|\text{spot} - \text{atm\_strike}\| < 50\text{ pts}$ (spot closely anchored to strike).
...
- Must have $\text{DTE} \in (0, 1)$.
- On 0DTE, entry after 12:00 IST is forbidden.
- Spot must be within 50 points of ATM strike.
- $\text{ADX}_{15}$ must be $\le 22$.
- Opening range condition cannot be `WIDE` or `VERY_WIDE`.

#### 2. Iron Condor Rules:

- **Runway Gate**: Minutes to hard exit must be $\ge 75\text{ min}$ (on 0DTE) or $\ge 90\text{ min}$ (on $\text{DTE} \ge 1$).
- **Momentum Gate**: $\text{ADX}_{15}$ must be below `adx_strong_threshold` (default 25) and $\text{ADX}_{15}$ must be mature.
...
- **Opening Range Mid-Positioning**: On 0DTE, condors cannot be entered if spot is too close to opening range boundaries. Spot must be centered:
  $$
  \text{OR}_{\text{low}} + 30 < \text{spot} < \text{OR}_{\text{high}} - 30
  $$

#### 3. Bull Put Spread Rules:

- **Location Check**: Spot cannot be deeply below Opening Range Midpoint ($\text{spot} \ge \text{OR}_{\text{mid}} - 30$ on 0DTE, $-15$ on $\text{DTE} \ge 1$).
...
- **Location Check**: Spot cannot be deeply above Opening Range Midpoint ($\text{spot} \le \text{OR}_{\text{mid}} + 30$ on 0DTE, $+15$ on $\text{DTE} \ge 1$).
...
- **Target Delta Assignment**:
  - Weekly ($\text{DTE} \ge 2$) Condors: Targets $\Delta \approx 0.18$ (if strong ADX) or $\Delta \approx 0.20$ (flat).
  - 0DTE / Near-dated: Targets $\Delta \approx 0.22 - 0.25$ (shaved down by $0.01-0.02$ if $\text{VIX} < 14$).
  - Directional Verticals: Retains canonical $\Delta \approx 0.28 - 0.32$ on the favoured side.
- **Expected Move Bounds Clamping**:
  Short strikes are clamped within an Expected Move (EM) band:
  $$
  [\text{band\_lo}, \text{band\_hi}] = [\text{em\_band\_lo} \times \text{EM}, \text{em\_band\_hi} \times \text{EM}]
  $$

  (Weekly bounds: $0.55 \times \text{EM}$ to $1.35 \times \text{EM}$ for condors; up to $2.10 \times \text{EM}$ for verticals).
- **Extreme-Fade Strike Cushioning**:
  - Failed-break verticals pin the short beyond the tested extreme plus cushion ($\ge 80\text{ pts}$).
  - High/Low fades pin the short strike at least $80\text{ pts}$ outside the confirmed extreme, bumping one strike step further if nearest rounding lands within the proximity stop band.
- **Adaptive Wing Width Fitting (`_fit_wing_width`)**:
  - On weekly condors ($\text{DTE} \ge 2$), multi-day vega makes protective wings expensive. The engine widens the wing step-by-step from base width up to $\text{wing\_max}$ until long premium $\le \text{wing\_cost\_frac\_max\_weekly}$ ($50\%$).
  - Wing factor is interpolated via continuous sqrt-life:

    $$
    \text{wing\_factor} = \text{by\_dte}(\text{DTE}, 0.50, 0.62)
    $$

    $$
    \text{wing\_max} = \text{round}\left(\frac{\text{by\_dte}(\text{DTE}, 250.0, 450.0)}{\text{step}}\right) \times \text{step}
    $$

    (Interpolates from $250\text{ pts}$ on 0DTE to $450\text{ pts}$ on $\text{DTE} \ge 2$).

#### 2. Pricing & Validation (`_build_validated_legs`):

For every leg:

- Validates bid $> 0$, ask $> 0$, and $\text{spread} \le \text{spread\_max\_pts}$.
- Execution Price Model:

  $$
  \text{Exec Price}_{\text{SELL}} = \text{bid} + \text{fill\_edge} \times (\text{ask} - \text{bid})
  $$

  $$
  \text{Exec Price}_{\text{BUY}} = \text{ask} - \text{fill\_edge} \times (\text{ask} - \text{bid})
  $$
...
- **Target Fraction (`_get_target_pct`)**:
  - Base target interpolated via `config.target_pct_for_dte(actual_dte)`:
    $$
    \text{target\_pct} = \text{by\_dte}(\text{DTE}, 0.70, 0.40)
    $$

    (Yields $\approx 0.70$ on 0DTE, $\approx 0.52$ on 1DTE, $0.40$ on $\text{DTE} \ge 2$, shaved by $-0.03$ for $\text{DTE} \ge 3$, and shaved further by $-0.035$ to $-0.07$ on elevated VIX $\ge 12$).
- **Stop Loss Multiplier (`stop_mult_for_dte`)**:
  - Base stop multiplier interpolated via `config.stop_mult_for_dte(actual_dte)`:
    $$
    \text{stop\_mult} = \text{by\_dte}(\text{DTE}, 1.60, 1.70)
    $$

    (Yields $1.60\times$ on 0DTE, $\approx 1.66\times$ on 1DTE, $1.70\times$ on $\text{DTE} \ge 2$, capped at structural wing loss).
...
$$
\text{Slippage}_{\text{entry}} = \sum_{\text{legs}} \frac{\text{ask}_i - \text{bid}_i}{2} \times \text{entry\_slippage\_mult (0.35)}
$$

$$
\text{Slippage}_{\text{exit\_stressed}} = \sum_{\text{legs}} \frac{\text{ask}_i - \text{bid}_i}{2} \times \text{exit\_slippage\_mult (2.25)}
$$
...
1. **Net Credit Positive**: $\text{Net Credit} = \text{Gross Credit} - \text{Entry Costs} - \text{Entry Slippage} > 0$.
...
5. **Credit-to-Wing Ratio**:
   $$
   \frac{\text{Net Credit}}{\text{Wing Width}} \ge \begin{cases} \text{MIN\_CREDIT\_RATIO\_DTE0}[\text{strat}] & \text{for 0DTE (0.13 IC, 0.18 Fly, 0.11 Verticals)} \\ \text{MIN\_CREDIT\_RATIO}[\text{strat}] & \text{for DTE} \ge 1 \text{ (0.10 IC, 0.15 Fly, 0.08 Verticals)} \end{cases}
   $$
6. **Profit vs Friction**: Expected profit ($\text{Net Credit} \times \text{Target \%}$) must be at least $1.75\times$ total round-trip friction.
...
1. **Target Win**: Position hits profit target cleanly ($p_{\text{win\_eff}}$).
2. **Stop Loss**: Position triggers the premium/spot stop ($p_{\text{stop}}$).
3. **Tail Loss / Jump**: Position suffers a fast volatility jump or gap through the stop toward the wing ($p_{\text{tail}}$).

$$
\text{EV} = p_{\text{win\_eff}} \cdot (\text{Reward} - \text{Friction}_{\text{calm}}) - p_{\text{stop}} \cdot (\text{Stop Loss} + \text{Friction}_{\text{stressed}}) - p_{\text{tail}} \cdot (\text{Tail Loss} + \text{Friction}_{\text{stressed}})
$$

Where:

- $\text{Tail Loss} = \text{Stop Loss} + 0.30 \times \max(\text{Wing Loss} - \text{Stop Loss}, 0)$.
- $p_{\text{tail}} = \text{config.gamma\_tail\_prob\_for\_dte}(\text{DTE}) = \text{by\_dte}(\text{DTE}, 0.055, 0.020)$ (clamped $\le 0.20$, scaled $1.8\times$ on wide OR and $1.5\times$ on strong ADX).
- $p_{\text{win\_eff}} = p_{\text{win}} \cdot (1 - p_{\text{tail}})$.
- $p_{\text{stop}} = 1 - p_{\text{win\_eff}} - p_{\text{tail}}$.

#### 2. Tri-Partite Win Probability ($p_{\text{win}}$) Formulation:

$$
p_{\text{win}} = w_m \cdot p_{\text{win\_model}} + w_p \cdot p_{\text{win\_prior}} + w_k \cdot p_{\text{mkt}}
$$

(Default weights: $w_m = 0.40$, $w_p = 0.30$, $w_k = 0.30$; on thesis reclaim verticals: $w_m = 0.25, w_p = 0.20, w_k = 0.55$).

- **Barrier Model ($p_{\text{win\_model}}$)**:
  Uses the true short strike distance $b = \|\text{strike} - \text{spot}\| - \text{barrier\_pull\_pts}$.

  $$
  \sigma_{\text{pts}} = \min(\sigma_{\text{iv}}, \sigma_{\text{straddle}} \times 1.15)
  $$

  $$
  z = \frac{b}{\sigma_{\text{pts}}}
  $$

  $$
  p_{\text{touch}} = \sum_{b \in \text{barriers}} 2 \cdot \Phi(-z)
  $$

  - *Max Pain Magnet Adjustment*: If $\text{DTE} == 0$ and spot is within $0.5 \times \sigma_{\text{pts}}$ of Max Pain, $p_{\text{touch}}$ is discounted by $15\%$ ($p_{\text{touch}} \leftarrow p_{\text{touch}} \times 0.85$).

  $$
  p_{\text{win\_model}} = \text{clamp}(1.0 - p_{\text{touch}}, 0.20, 0.93)
  $$
- **Market Delta Prior ($p_{\text{mkt}}$)**:

  $$
  p_{\text{mkt}} = 1.0 - \max(|\Delta_{\text{short\_legs}}|)
  $$
- **Empirical Table Prior ($p_{\text{win\_prior}}$)**:
  Indexed by DTE and Opening Range Condition (`VERY_NARROW`, `NARROW`, `MODERATE`, `WIDE`, `VERY_WIDE`), adjusted by smoothed VRP bonus ($+0.05$ if $\text{VRP} > 3.5\text{ pp}$).

#### 3. EV Rejection Threshold:

$$
\text{Min EV} = \max(\text{Net Credit} \times 0.03, \text{Friction} \times 0.35)
$$

If $\text{EV} < \text{Min EV}$, the parameter package is marked `valid: False` with `ev_gate:ev_X_below_min_Y`.
...
- $\text{Life}_{\text{expiry}} = 0.5\text{ sessions}$ ($\sqrt{0.5} \approx 0.7071$)
- $\text{Life}_{\text{weekly}} = 2.5\text{ sessions}$ ($\sqrt{2.5} \approx 1.5811$)

For any integer or floating DTE:

$$
\text{life} = \text{DTE} + 0.5
$$

$$
w = \frac{\sqrt{2.5} - \sqrt{\text{life}}}{\sqrt{2.5} - \sqrt{0.5}} = \frac{1.5811 - \sqrt{\text{DTE} + 0.5}}{0.8740}, \quad w \in [0.0, 1.0]
$$

$$
\text{by\_dte}(\text{DTE}, V_{\text{expiry}}, V_{\text{weekly}}) = V_{\text{weekly}} + w \cdot (V_{\text{expiry}} - V_{\text{weekly}})
$$

Evaluating weights:

- **0DTE**: $w = 1.0 \rightarrow$ Exact expiry-day parameter.
- **1DTE**: $w \approx 0.41 \rightarrow$ Smoothly interpolates 41% towards expiry anchor.
- **$\ge$ 2DTE**: $w = 0.0 \rightarrow$ Exact weekly anchor.
...
| ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------ | -------------------------------------------------------- | ---------------------------------------- | --------------------------------------------- | ----------------------------------------- |
| **Interpolation Weight ($w$)**                 | $1.00$                                                                                               | $\approx 0.41$                                         | $0.00$                                 | $0.00$                                      | `core.dte_blend(dte)`                   |
...
| **Gamma Tail Probability ($p_{\text{tail}}$)** | $0.055$ ($5.5\%$)                                                                                  | $\approx 0.035$ ($3.5\%$)                            | $0.020$ ($2.0\%$)                    | $0.020$ ($2.0\%$)                         | `config.gamma_tail_prob_for_dte(dte)`   |
...
| **Target Net Credit \%**                         | $0.70$ ($70\%$)                                                                                    | $\approx 0.52$ ($52\%$)                              | $0.40$ ($40\%$)                      | $0.37$ ($37\%$, $-0.03$ shave)          | `config.target_pct_for_dte(dte)`        |
| **Stop Loss Multiple**                           | $1.60\times$ net credit                                                                              | $\approx 1.66\times$ net credit                        | $1.70\times$ net credit                | $1.70\times$ net credit                     | `config.stop_mult_for_dte(dte)`         |
| **Delta Close Threshold**                        | $\Delta \ge 0.45$                                                                                    | $\approx 0.48$                                         | $\Delta \ge 0.50$                      | $\Delta \ge 0.50$                           | `config.delta_close_for_dte(dte)`       |
| **EV Carry Discount**                            | $1.00$ (No theta carry credit)                                                                       | $\approx 0.78$                                         | $0.65$ (`ev_carry_discount_dte2p`)   | $0.65$                                      | `config.ev_carry_discount_for_dte(dte)` |
| **Wing Factor & Max Wing Width**                 | Factor: 0.50, Max:$250\text{ pts}$                                                                   | Factor:$\approx 0.57$, Max: $\approx 370\text{ pts}$ | Factor: 0.62, Max:$450\text{ pts}$     | Factor: 0.62, Max:$450\text{ pts}$          | Adaptive wing clamping via`by_dte`      |
| **Margin Add-on Buffer**                         | $+18\%$ broker tightening                                                                            | $+10\%$ buffer                                         | $+10\%$ buffer                         | $+10\%$ buffer                              | Fully hedged lot margin model             |
| **Opening Range Buffer**                         | $\pm 30\text{ pts}$ from OR mid                                                                      | $\pm 15\text{ pts}$ from OR mid                        | $\pm 15\text{ pts}$ from OR mid        | $\pm 15\text{ pts}$ from OR mid             | `_validate_entry_rules`                 |
...
2. **0DTE Ban**: $\text{DTE} == 0$ is strictly blocked due to explosive afternoon theta decay. Requires $\text{DTE} \in [1, 4]$.
...
4. **Strong Trend Alignment**:
   - Morning Route ($< 14:00$ IST): Requires confirmed `price_regime` in `UPTREND`, `STRONG_UPTREND`, `DOWNTREND`, or `STRONG_DOWNTREND`. Requires $\text{ADX}_{15} \ge 25$ and spot breaking the opening range boundary.
   - Closing-Hour Route ($\ge 14:00$ IST): Controlled by `_in_late_momentum_window`. Requires EMA 9/21 alignment, fresh session high/low extreme within 30 min, and tape displacement $> 35\text{ pts}$.
5. **No Volatility Top**: India VIX gap $\le 12\%$ and IV behavior not spiking.

#### 2. Momentum Strike Selection & Economics (`compute_momentum_params`):

- Direction: $+1 \rightarrow$ **`LONG_CALL`**, $-1 \rightarrow$ **`LONG_PUT`**.
- Delta Target: Delta $\approx 0.45 - 0.55$ (slightly In-The-Money or exact At-The-Money, maximizing gamma while controlling spread friction).
- **Hard Edge Hurdle**:

  $$
  \text{Expected Capture} = \text{Option Premium} \times \text{momentum\_target\_frac (0.35)}
  $$

  $$
  \text{Expected Capture} \ge \text{Friction} \times 2.0
  $$
...
- **`IRON_CONDOR`**: Primary range harvester on quiet/normal sessions ($\text{DTE} \ge 0$). 4 legs (Short OTM Put + Long Far OTM Put, Short OTM Call + Long Far OTM Call). Delta-neutral.
...
- **`LONG_CALL` / `LONG_PUT`**: Intraday momentum substitutes on trend expansion days ($\text{DTE} \in [1, 4]$). 1 leg (Long ATM/Slightly ITM option). Net debit.
...
- **Stop Efficacy Blend**:
  ...
  $$
  \text{Structural Loss per Lot} = \text{efficacy} \cdot \text{Stop Loss} + (1 - \text{efficacy}) \cdot \text{Wing Loss}
  $$
...
- **Risk Budget**:
  ...
  $$
  \text{Max Risk} = \text{Current Capital} \times \text{max\_risk\_per\_trade\_pct (0.006)}
  $$

  $$
  \text{Raw Lots} = \frac{\text{Max Risk}}{\text{Structural Loss per Lot}}
  $$

  $$
  \text{Sized Lots} = \text{Raw Lots} \times \text{size\_mult}
  $$
- **Minimum Economic Fraction (`_min_frac = 0.60`)**:

  - If $\text{Sized Lots} < 0.60$: Triggers minimum-ticket affordability check. If $\text{Raw Lots} \ge 1.0$ on clear setup with non-unclear positioning, clips to 1 lot. Otherwise rejects: `risk_budget_allows_only_X_lots_below_min_0.60`.
- **Fixed-Cost Amortization Floor (`v3.10 [G6]`)**:

  - On HIGH-confidence setups with thin gross capture, if $\text{final\_lots} == 1$ but $\text{Raw Lots} \ge 1.5$ (and non-event day), sizes up to $\min(\text{round}(\text{raw\_lots}), \text{day\_cap}, 3)$ so fixed order brokerage is amortized.
- **Daily Day Cap Table (`LOT_CAPS_BY_DAY`) Scaled by Equity**:

  $$
  \text{Day Cap} = \max(1, \lfloor \text{LOT\_CAPS\_BY\_DAY}[\text{day}] \times \sqrt{\text{Capital} / \text{Starting Capital}} \rfloor)
  $$
...
- **Capital Margin Constraint**:
  ...
  $$
  \text{Wing Margin per Lot} = \text{Wing Width} \times \text{Lot Size (65)} \times 1.10 \times (1 + \text{addon})
  $$

  $$
  \text{Total Margin} = \text{Margin per Lot} \times \text{Lots} \le \text{Capital} \times 0.80
  $$
...
- **`FillModel`**: Two-sided spread-aware execution simulation:
  ...
  $$
  \text{Price}_{\text{SELL}} = \text{bid} + (\text{mid} - \text{bid}) \cdot \frac{\text{edge}}{0.5}
  $$

  $$
  \text{Price}_{\text{BUY}} = \text{ask} - (\text{ask} - \text{mid}) \cdot \frac{\text{edge}}{0.5}
  $$

  On urgent exits (priority stops 1, 2, 3, 7), $\text{edge} \leftarrow \text{edge} \cdot \text{stress\_mult}$ (default $0.25 \times 0.50 = 0.125$).
...
3. `MarketDataEngine.run_cycle()` $\rightarrow$ Updates indicators, rolling VWAP, and ATM straddles.
4. `BacktestRunner._classify()` $\rightarrow$ Calls `regime.process_signals()` and merges regime snapshot.
...
2. `entry window`: Time $< 09:20$, time $> 14:15$, runway to hard exit $< 50-90\text{ min}$.
...
- `vix_spike_detected`: India VIX jumped $\ge 15\%$ intraday.
...
- `butterfly_spot_too_far_from_atm_X`: Spot deviated $> 50\text{ pts}$ from ATM strike.
- `butterfly_requires_dte_0_or_1_not_X`: Iron Butterfly attempted on $\text{DTE} \ge 2$.
...
- `condor_blocked_strong_adx_X`: Iron Condor attempted with $\text{ADX} \ge 25$.
...
- `momentum_adx_X_below_min`: Trend strength insufficient ($\text{ADX} < 25$).
...
- `momentum_expected_capture_Xpts_below_Yx_friction`: Long option target gain does not cover twice round-trip execution frictio
...
1. [Executive Summary &amp; Core Design Philosophy](#1-executive-summary--core-design-philosophy)
2. [Module Architecture &amp; Import Dependency Graph](#2-module-architecture--import-dependency-graph)
3. [Engine Initialization, Data Structures &amp; Lifecycle](#3-engine-initialization-data-structures--lifecycle)
4. [Master Pipeline Lifecycle: `decide()`](#4-master-pipeline-lifecycle-decide)
5. [Hard Gate Validation Engine (`_check_hard_gates`)](#5-hard-gate-validation-engine-_check_hard_gates)
6. [Regime Classification &amp; Regime-to-Strategy Mapping](#6-regime-classification--regime-to-strategy-mapping)
7. [Range Strategy Resolution &amp; Two-Way Extreme Fading Logic](#7-range-strategy-resolution--two-way-extreme-fading-logic)
...
### 3. Engine Initialization, Data Structures & Lifecycle
...
- `session_high`, `session_low`: Dynamic intraday extreme prices.
- `session_mean_reversion_book`: Boolean flag set after a completed high/low fade scalp.
- `construct_fail`: Latch tracking repeated economic/structural leg failures on static auction prints.
...
6. **Sticky Construct Check (`_sticky_construct_reason`)**: Prevents continuous order building when the auction is stationary and previously failed economics.
7. **Parameter Computation & EV Gating (`compute_params`)**: Selects strikes, builds legs, calculates round-trip friction, applies EV barrier simulation, and checks risk budgets.
   - **Condor Demotion**: If an Iron Condor fails economics, attempts demotion to a single-sided Bull Put or Bear Call spread matching the session lean (`_demote_condor_on_econ_fail`).
   - **Momentum Substitution**: If parameters are invalid, passes the structural failure token to `_momentum_decision`.
8. **Final Decision Dispatch**: Persists decision, updates state, and returns action payload:
   ```json
   {
